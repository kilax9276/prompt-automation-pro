...
### Срез 2, блок 2+5 — написан, НЕ закоммичен, НЕ выкачен

Это текущая работа. Двадцать шесть раундов проверки параллельным ревьюером, в
каждом находились и устранялись дефекты; review 27 подготовлен к следующей независимой проверке. Состояние на момент последнего пакета:

```
235 тестов passed, 64 subtests
py_compile чистый, node --check чистый
```

Числа ниже в этом документе относятся к состоянию тринадцатого раунда, когда он
был написан, и обновляются здесь, а не переписываются задним числом по всему
тексту: раздел «Что проверено» и раздел передачи приведены к текущей базе.

Изменены три файла относительно `8fc773c`:
`endpoint_registry.py`, `console_server.py`, `delivery_manager.py`
(плюс ранее `static/app.js` — удаление интерфейса закрепления).
...
Читаемые хвосты `pin_state`, `pinned_tab`, `select_for_binding` уходят вместе
с этим шагом. `ACC-S2-008` получает PASS только после него.
...
tests/                          235 тестов, пятнадцать файлов
diff/                           три патча относительно коммита 8fc773c
docs/PAP2_4.5.0_DESIGN.md       дизайн, 2645 строк, журнал решений
...
шаг 2+5 модель endpoint, владение, барьеры,
        DRAIN/BARRIER                             ← ЗДЕСЬ, ожидает подтверждения
шаг 3   selectEndpoint, привязка сессии
...
**Немедленно:** дождаться подтверждения ревьюера по блоку 2+5. Если найдутся
новые дефекты — устранять их в том же незакоммиченном наборе, не переходя
дальше.

**После подтверждения:** зафиксировать блок 2+5 отдельным development-коммитом
поверх `8fc773c`, без выката. Стенд остаётся на срезе 1.

**Затем:** шаг 3, `selectEndpoint` как отношение сессии и привязки
(`MAP-028`), и только после него удаление остатков старого выбора
(`MAP-030/033/049/050`) — нельзя убирать способ выбора раньше, чем появился
новый.
...
Ожидается `235 passed, 64 subtests passed` и актуальные нулевые результаты трёх verify-инструментов.
...
1. Не выкатывать и не коммитить в настоящий репозиторий, пока блок 2+5 не
   признан целостным. Стенд работает на срезе 1.
...
**Текущая задача:** ожидается подтверждение ревьюера по блоку 2+5. Если
пользователь принесёт новые замечания — воспроизведи каждый дефект на коде
до починки, устрани, закрепи тестом, воспроизведи после, покажи оба прогона.
Если подтверждение получено — зафиксируй блок коммитом и переходи к шагу 3,
`selectEndpoint` (контракт в разделе 5c).
...
сервер и расширение выкатываются только вместе. Его шаг 1 закоммичен, а
серверный блок 2+5 написан и прошёл двадцать шесть раундов внешней проверки, но
ещё не закоммичен и не выкачен: review 27 локально зелёный и ожидает независимого подтверждения.
